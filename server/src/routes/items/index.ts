...
router.post('/', async (req, res) => {
  const params = insertableColumns.map(
    (col) => (req.body as Record<string, unknown>)[col]?.toString?.()?.trim?.() ?? (req.body as Record<string, unknown>)[col]
  )
  const missing = insertableColumns.filter((col, i) => params[i] === undefined || params[i] === '')
  if (missing.length > 0) {
    res.status(400).json({ error: `${missing.join(', ')} is required` })
    return
  }
  try {
