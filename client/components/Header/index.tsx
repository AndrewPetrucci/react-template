...
const THEME_OPTIONS: ButtonListItem[] = [
...
export default function Header() {
  ...
  const { user, meChecked } = useSelector((state: RootState) => state.auth)
  ...
  useEffect(() => {
    const stored =
      typeof document !== 'undefined' && localStorage.getItem(THEME_KEY)
  ...
  const handleThemeSelect = (item: ButtonListItem | string) => {
    const value = typeof item === 'object' && item && 'value' in item ? String(item.value) : String(item)
    setTheme(value)
